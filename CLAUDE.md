...
- Touching data/dashboard? Read `scripts/build_dashboard.py` and the view definitions in Databricks.
...
| `scripts/build_dashboard.py` | Builds the Lakeview "Strategist Impact Dashboard (Felix)" via the Lakeview REST API |
...
- Embed the AI/BI Impact Dashboard in a new `/impact` route (iframe or Lakeview embed SDK) — the backing dashboard already exists via `scripts/build_dashboard.py`.
